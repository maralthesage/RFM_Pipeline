...
v21056 = pd.read_csv(
    rechnung_path_f04, sep=";", encoding="cp850", parse_dates=["AUF_ANLAGE"]
)
...
kw = pd.read_csv(
    "Data/kw_f04.csv", sep=";", encoding="cp850", usecols=["NUMMER", "Kundengruppe"]
)
...
filtered_final_merged.to_csv('/Volumes/MARAL/Data/rfm_labels/rfm_labels_f04.csv', sep=';', index=False, encoding='cp850')
with pd.ExcelWriter(f"Data/rfm_segments_{today.date()}_f04.xlsx", engine="xlsxwriter") as writer:
    for item in kundengruppe:
...
with pd.ExcelWriter(
    f"Data/rfm_segments_gesamt_{today.date()}_f04.xlsx", engine="xlsxwriter"
) as writer:
    # Write both tables
