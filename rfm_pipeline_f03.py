...
v21056 = pd.read_csv(
    rechnung_path_f03, sep=";", encoding="cp850", parse_dates=["AUF_ANLAGE"]
)
...
kw = pd.read_csv(
    "Data/kw_f03.csv", sep=";", encoding="cp850", usecols=["NUMMER", "Kundengruppe"]
)
...
address_details["AUF_ANLAGE"] = address_details["AUF_ANLAGE"].fillna(pd.Timestamp("1800-01-01"))  # Fill NaT with a default date
address_details.loc[
    (address_details["AUF_ANLAGE"] == pd.Timestamp("1800-01-01"))
    & (address_details["SYS_ANLAGE"].dt.year >= dt.date.today().year - 1), "Kundengruppe"] = "Neu-Interessenten"
address_details.loc[
    (address_details["AUF_ANLAGE"] == pd.Timestamp("1800-01-01"))
    & (address_details["SYS_ANLAGE"].dt.year <= dt.date.today().year - 1),"Kundengruppe"] = "Alt-Interessenten"
...
addresses_grouped.loc[addresses_grouped["gesamt_frequency"] == 1, "seasonal_ostern"] = (
    False
)
addresses_grouped.loc[
    addresses_grouped["gesamt_frequency"] == 1, "seasonal_weihnachten"
] = False
addresses_grouped.loc[
...
address_details_2 = address_details[
    (address_details["AUF_ANLAGE"] >= two_years_ago_start)
    & (address_details["AUF_ANLAGE"] < today)
]
...
addresses_details_last5years = addresses_grouped.merge(
    last_5_years, on="NUMMER", how="left"
)
...
addresses_details_last5years["monetary_last_2_years"] = addresses_details_last5years[
    "monetary_last_2_years"
].clip(lower=0)
addresses_details_last5years["monetary_3_to_5_years_ago"] = (
    addresses_details_last5years["monetary_3_to_5_years_ago"].clip(lower=0)
)
addresses_details_last5years["gesamt_monetary"] = addresses_details_last5years[
    "gesamt_monetary"
].clip(lower=0)


### ============== Assigning Age Groups and Anrede and trasnlating Quelle into Sources ============== ###
final_addresses = addresses_details_last5years.sort_values(
    by="gesamt_frequency", ascending=False
)
...
final_addresses["freq_last_5_years"] = round(
    (
        (final_addresses["freq_3_to_5_years_ago"] * 0.5)
        + (final_addresses["freq_last_2_years"])
    )
)
...
final_addresses["recency"] = pd.to_datetime(
...
final_addresses["monetary_last_5_years"] = final_addresses[
    "monetary_last_5_years"
].clip(lower=0)
...
final_addresses.loc[final_addresses["gesamt_monetary"] == 0, "rfm_label"] = (
    "Interessenten"
)


final_addresses = final_addresses.drop(
    columns=["kundengruppe"]
)  ## Remove Kundengruppe column
...
filtered_final_merged.to_csv('/Volumes/MARAL/Data/rfm_labels/rfm_labels_f03.csv', sep=';', index=False, encoding='cp850')
with pd.ExcelWriter(f"Data/rfm_segments_{today.date()}_f03.xlsx", engine="xlsxwriter") as writer:
    for item in kundengruppe:
...
with pd.ExcelWriter(
    f"Data/rfm_segments_gesamt_{today.date()}_f03.xlsx", engine="xlsxwriter"
) as writer:
    # Write both tables
