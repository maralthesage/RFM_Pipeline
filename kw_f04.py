...
result = get_half_year_info(land='CH')
...
kw = pd.read_csv(kw_path_f04, sep=sep, encoding=enc, on_bad_lines="skip")
adresse = pd.read_csv(address_path_f04, sep=sep, encoding=enc)
stat = pd.read_csv(stat_path_f04, sep=sep, encoding=enc, usecols=["NUMMER", "ERSTKAUF"])
...
all_addresses_labeled[["NUMMER", "Kundengruppe"]].to_csv(
    f"Data/kw_f04.csv", sep=";", index=False, encoding="cp850"
)
